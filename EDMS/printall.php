...
$From = "FROM `{$_SESSION['DBCoy']}`.`edms` 
    INNER JOIN `{$_SESSION['DBCoy']}`.`sch_schemes` ON `edms`.`scheme`=`sch_schemes`.`schm_id`";

$sql = "SELECT `edms`.*, `schm_name` {$From}{$orderval}";
...
<table border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><table border="0" cellspacing="2" cellpadding="2">
        <tr>
          <td><img src="<?php echo COYPIX_DIR, $_SESSION['../admin/sessions/coyid']."/xxpix.jpg" ?>" /></td>
...
onmouseover="setPointer(this, <?php echo $j ?>, 'over', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onmouseout="setPointer(this, <?php echo $j ?>, 'out', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onclick="location.href='view.php?id=<?php echo $row_TSess['doc_id']; ?>'">
