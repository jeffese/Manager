...
//	function _nbrConversionFormat()
     ...
     *
     */
    public static function BESSELI($x, $ord, $asc) {
        ...
        if ((is_numeric($x)) && (is_numeric($ord))) {
            ...
            if (abs($x) <= 30) {
                $fResult = $fTerm = pow($x / 2, $ord, $asc) / PHPExcel_Calculation_MathTrig::FACT($ord);
                $ordK = 1;
                $fSqrX = ($x * $x) / 4;
                do {
                    $fTerm *= $fSqrX;
                    $fTerm /= ($ordK * ($ordK + $ord, $asc));
                    $fResult += $fTerm;
...
//	function BESSELI()
     ...
     *
     */
    public static function BESSELJ($x, $ord, $asc) {
        $x = (is_null($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($x);
        $ord = (is_null($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($ord);
...
//	function _Besselk1()
     ...
     *
     */
    public static function BESSELK($x, $ord, $asc) {
        $x = (is_null($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($x);
        $ord = (is_null($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($ord);
...
//	function _Bessely1()
     ...
     * 	@return	float
     */
    public static function BESSELY($x, $ord, $asc) {
        $x = (is_null($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($x);
        $ord = (is_null($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue($ord);
