...
INNER JOIN `{$_SESSION['DBCoy']}`.`classifications`  `par` ON `classifications`.`Dept`=`par`.`catID`
LEFT JOIN `{$_SESSION['DBCoy']}`.`status` ON `classifications`.`cat_tag`=`status`.`CategoryID` 
WHERE `classifications`.`OutletID`=$id";
              ...
              <tr>
                <td><table border="0" cellpadding="4" cellspacing="4">
                  ...
                  <tr>
                    ...
                    <td><?php echo $row_TOutlets['description'] ?></td>
                  </tr>
                </table></td>
              </tr>
              <tr>
