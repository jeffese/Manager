...
qryfind($vcat, array('name'));
preOrd($vcat, array('', 'OutletCode', 'OutletName', 'catname'));

$From = "FROM `{$_SESSION['DBCoy']}`.`outlets`
LEFT JOIN `{$_SESSION['DBCoy']}`.`classifications` ON `outlets`.`Dept`=`classifications`.`catID`
WHERE account=$vtype {$qryvals}";

$sql = "SELECT OutletID, OutletCode, OutletName, catname {$From}{$orderval}";
...
onmouseover="setPointer(this, <?php echo $j ?>, 'over', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onmouseout="setPointer(this, <?php echo $j ?>, 'out', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onclick="location.href='view.php?id=<?php echo $row_TOutlets['OutletID']; ?>'">
                                  <td align="center" nowrap="nowrap" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><b><?php echo $row_TOutlets['OutletCode'] ?></b></td>
                                  <td align="center" nowrap="nowrap" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><b><?php echo $row_TOutlets['OutletName'] ?></b></td>
                                  <td align="center" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><b><?php echo $row_TOutlets['catname'] ?></b></td>
                                </tr>
                                <?php $j++;} ?>
                              </table></td>
                            </tr>
