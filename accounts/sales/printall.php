...
qryfind("sale");
preOrd('sale', array('', 'InvoiceID', 'VendorName', 'Grandvalue', 'InvoiceDate', 'Posted'));
...
$From = "FROM `{$_SESSION['DBCoy']}`.`invoices` 
    ...
    LEFT JOIN `{$_SESSION['DBCoy']}`.`vendors`                 ON `invoices`.VendorID = vendors.VendorID
    WHERE `invoices`.OutletID IN ($outid) {$qryvals}";

$sql = "SELECT `InvoiceID`, `InvoiceDate`, `Grandvalue`, Posted, $vendor_sql {$From}{$orderval}";

$TSales = getDBData($dbh, $sql);
                            ...
                            <tr>
                              <td align="center" valign="top" bgcolor="#FFFBF0"><table width="100%" cellpadding="4" cellspacing="1" style="border: 2px #CCCCCC">
                                <tr align="center" bgcolor="#666666" class="boldwhite1">
                                  ...
                                  <td nowrap="nowrap"><?php echo setOrderTitle('Customer', $currentPage, 3, $ord, $asc); ?></td>
