...
$buttons_links = array("","","","","","edit.php?id=$id","","[Sale]del.php?id=$id","../kiosk.php","","","|print.php?id=$id","");
$rec_status = 1;
...
//            throw new Exception("Insufficient Funds!");
        ...
        $sql = "UPDATE `{$_SESSION['DBCoy']}`.`invoicedetails`
                ...
                INNER JOIN `{$_SESSION['DBCoy']}`.`items_prod`  ON `invoicedetails`.ProductID=`items_prod`.`ProductID`
                INNER JOIN `{$_SESSION['DBCoy']}`.`outlet`      ON `invoices`.`OutletID`=`outlet`.`OutletID` 
                                                                AND `invoicedetails`.ProductID=outlet.ProductID
        ...
        $sql = "UPDATE `{$_SESSION['DBCoy']}`.`invoicedetails`
                ...
                INNER JOIN `{$_SESSION['DBCoy']}`.`outlet`          ON `invoices`.`OutletID`=`outlet`.`OutletID` 
                                                                    AND `items_prod`.`ProductID`=outlet.ProductID
                SET `ShopStock`=`ShopStock`-`units`
                WHERE `invoices`.`InvoiceID`=$id";
        runDBQry($dbh, $sql);
        ...
        $sms_vars = getDBDataRow($dbh, 
                "SELECT `SrvSchedID`, `enddate`, `items_srv_sched`.`MachineTime`, `licenceno`, `PhoneNumber` 
                ...
                WHERE `invoicedetails`.`InvoiceID`=$id AND `ProductID`=61");
...
if (_xpost("MM_Post") == "frmpost" && strlen($row_TSales['ShipTo']) > 3) {
    $sql = "UPDATE `{$_SESSION['DBCoy']}`.`items_srv_sched` 
        SET `Status`=NULL WHERE `SrvSchedID`=" . $row_TSales['ShipTo'];
    runDBQry($dbh, $sql);
}
//$_bottons array(new, edit, delete, print, Nav, find)
$_bottons = array(0, AccStat($access['Edit'], $row_TSales['Posted']), AccStat($access['Del'], $row_TSales['Posted']), $access['Print'], 0, 0, 0, 0, 1);
?>
...
<table width="100%" border="0" cellspacing="4" cellpadding="4">
              <tr>
                <td><table border="0" cellpadding="4" cellspacing="4">
                  <tr>
                    <td></td>
                    <td align="center"><?php echo catch_error($errors); echo showMsg($xMessages) ?></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td align="left"><input type="checkbox" name="Posted"<?php if ($row_TSales['Posted'] == 1) {
                echo " checked=\"checked\"";
                  ...
                  <tr>
                    <td height="28" colspan="2" align="center"><?php if ($access['Post'] == 1 && $row_TSales['Posted'] == 0) { ?>
                      ...
                      <?php } else { ?>
                      <a id="post" href="/custom/reprint.php?id=<?php echo strlen($row_TSales['ShipTo']) > 4 ? $row_TSales['ShipTo'] : $row_TSales['InvoiceID']; ?>"><img src="/images/but_print.png" width="60" height="20" /></a>
                      <?php } ?></td>
                  </tr>
                </table></td>
              </tr>
