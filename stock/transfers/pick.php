...
preOrd("prod", array('', 'ProdName', 'Category', 'catname', 'ShopStock'));

$outid = _xvarloopstore("outid", "req_out");
$From = "FROM `{$_SESSION['DBCoy']}`.`items` 
            ...
            WHERE `items`.`InUse`=1 AND OutletID=0$outid";

$sql = "SELECT `items_prod`.`ProductID`, `ProdName`, status.Category, catname, ShopStock, 
        unitsinpack, `outlet`.`serials`, `serialized`
        {$From}{$orderval}";
...
onclick="parent.parent.appendProd(<?php echo "'", $row_TProducts['ProductID'], "','", str_replace(array('"', "'"), array('”', "’"), $row_TProducts['ProdName']), "','", $row_TProducts['ShopStock'], "','", $row_TProducts['serials'], "','", $row_TProducts['serialized'], "'" ?>)">
                                <td align="center" nowrap="nowrap" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><b><?php echo $row_TProducts['ProdName'] ?></b></td>
                                <td align="center" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><b><?php echo $row_TProducts['Category'] ?></b></td>
