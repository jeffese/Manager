...
$From = "FROM `{$_SESSION['DBCoy']}`.`sch_programs` INNER JOIN `{$_SESSION['DBCoy']}`.`sch_schemes` ON `sch_programs`.`scheme`=`sch_schemes`.`schm_id`";

$sql = "SELECT `sch_programs`.*, `schm_name` {$From}{$orderval}";

$TProg = getDBData($dbh, $sql);
...
onmouseover="setPointer(this, <?php echo $j ?>, 'over', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onmouseout="setPointer(this, <?php echo $j ?>, 'out', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onclick="location.href='view.php?id=<?php echo $row_TProg['prog_id']; ?>'">
                                  <td align="center"><b><?php echo $row_TProg['prog_name'] ?></b></td>
                                  <td align="center"><b><?php echo $row_TProg['catname'] ?></b></td>
                                  <td align="center"><b><?php echo $row_TProg['cert_name'] ?></b></td>
                                  <td align="center"><b><?php echo $row_TProg['class_no'] ?> <b><?php echo $row_TProg['class_pfx'] ?></b></b></td>
                                  </tr>
                                <?php $j++;} ?>
                              </table></td>
                            </tr>
