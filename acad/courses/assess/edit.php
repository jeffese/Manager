...
$buttons_links = array("","","","","","","frmasses","","view.php?id=$id","","","","");
$rec_status = 3;
...
isTeacher($arm, $course, $term, true);
viewTerm($term, true);
...
if (_xpost("MM_update") == "frmasses") {
    ...
    for ($q=0; $q<$qry_cnt; $q++) {
        ...
        $attach = explode(":", _xpost("attach_$q"));
        $prvfile = newfile(ROOT . ASSESS_DIR, "answers".DS.$student, $assess_id, $attach, "preview_{$q}_", "pv_{$q}_", "prvw_{$q}");
        
        $sql = sprintf("UPDATE `%s`.`sch_assess` SET `scores`=%s, `attachs`=%s, `comments`=%s, `Notes`=%s WHERE `assess_id`=%s",
                       $_SESSION['DBCoy'],
                       GSQLStr(_xpost("scores_$q"), "text"),
                       GSQLStr($prvfile['prvcode'], "text"),
...
course_sheet($arm, $cls, $term, $course, false);
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top"><table width="100%" border="0" cellspacing="4" cellpadding="4">
          ...
          <form action="<?php echo $editFormAction; ?>" onsubmit="collateFiles(); return true" method="post" enctype="multipart/form-data" name="frmasses" id="frmasses">
...
onmouseover="setPointer(this, <?php echo $j ?>, 'over', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" onmouseout="setPointer(this, <?php echo $j ?>, 'out', '<?php echo $rowdefcolor ?>', '#CCFFCC', '#FFCC99');" id="row_<?php echo $j ?>">
                      ...
                      <?php
                        ...
                        for ($m = 0; $m < count($Per); $m++) {
                            ...
                            ?>
                            <td bgcolor="<?php echo $rowdefcolor ?>"><input name="ca_<?php echo $j, '_', $m ?>" type="text" id="ca_<?php echo $j, '_', $m ?>" value="<?php echo $val; ?>" size="3" style="text-align:right"<?php echo $readonly ?> onchange="numme(this); cummulate(<?php echo $j ?>);SetPosition()" /></td>
                            <?php
                    if ($grp_i != 0 && $grp_i != $GRP[$m+1]) {
                        $grp_i = $GRP[$m+1];
                        ?>
                        <td bgcolor="<?php echo $rowdefcolor ?>"><input name="grp_<?php echo $j, '_', $GRP[$m] ?>" type="text" id="grp_<?php echo $j, '_', $GRP[$m] ?>" value="" size="3" style="text-align:right" readonly="readonly" /></td>
                        <?php }} ?>
                      <td align="center" bgcolor="<?php echo $rowdefcolor ?>" class="black-normal"><input name="total_<?php echo $j ?>" type="text" id="total_<?php echo $j ?>" size="3" style="text-align:right" readonly="readonly" /></td>
