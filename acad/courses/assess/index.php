...
<script charset="UTF-8" src="<?php echo $ass == 'cls' ? "../../programs/classes/" : "../" ?>menu.js" type="text/javascript"></script>
...
<table border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="240" valign="top"><img src="/images/assess.jpg" width="240" height="300" />
                      ...
                      tree.attachEvent("onDblClick",function(id){
                          ...
                          if (typ == <?php echo $ass == 'cls' ? 5 : 6 ?>) {
                            ...
                            if (typ == 6) {
                                ...
                                cls_id = tree.getUserData(par2, 'id');
                                cls = tree.getItemText(par4) + ' > ' + 
                                    tree.getUserData(par3, 'code') + ' > ' + 
                                    tree.getUserData(par2, 'code');
                                location.href = '<?php echo $ass == 'att' ? 'attend/' : '' ?>view.php?cid=' + cls_id + 
                                    '&aid' + arm_id + '&cls=' + escape(cls) + '&tid=' + term_id + '&trm=' + escape(term) + 
                                    '&crsid=' + crs_id + '&arm=' + arm + '&ass=<?php echo $ass ?>';
                            } else {
                                ...
                                cls_id = tree.getUserData(par1, 'id');
                                cls = tree.getItemText(par2) + ' > ' + 
                                    tree.getUserData(par1, 'code');
                                location.href = '<?php echo $eval == 1 ? 'evaluations' : 'attend' ?>/view.php?cid=' + cls_id + 
                                    '&aid' + arm_id + '&cls=' + escape(cls) + '&tid=' + term_id + '&trm=' + escape(term) +
                                    '&arm=' + arm + '&ass=<?php echo $ass ?>';
                            }
                        } else {
                            tree.openItem(id);
                        }
