...
$url_chk = _xvarloopstore("chk", "cls_pick_xml");
$url_chk = explode('|', $url_chk);
...
foreach ($Tcat as $row_Tcat) {
    ...
    $img = $row_Tcat["typ"] == 4 ? ' im0="iconUncheckAll.gif"' : "";
    $chk = "";
    if ($row_Tcat["typ"] == 4 && in_array($row_Tcat["id"], $url_chk)) {
        $chk = ' checked="checked"';
        $img =  ' im0="iconCheckAll.gif"';
    }
    print('<item child="' . $ch . '"' . $img . $chk . ' id="' . $cat_id . '#' . $row_Tcat["id"] . '#' . $row_Tcat["typ"] . '" text="' . $catname . '"><userdata name="type">' . $row_Tcat["typ"] . '</userdata><userdata name="id">' . $row_Tcat["id"] . '</userdata><userdata name="code">' . $cat_id . '</userdata></item>');
}
print("</tree>");
?>